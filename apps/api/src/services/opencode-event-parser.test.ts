...
describe("parseOpenCodeEvent", () => {
  ...
  describe("opencode 1.14.20 run --format json events (real captured shapes)", () => {
    // Shapes captured verbatim from opencode 1.14.20 `run --format json`
    // inside the repo pod (ses_f75374e09ffepLY1K38ctZscdd etc.). The parser
    // previously expected a different (older) schema and silently dropped
    // every line, which made successful agent runs classify as no_output.
